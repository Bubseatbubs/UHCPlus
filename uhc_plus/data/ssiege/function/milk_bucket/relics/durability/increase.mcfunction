$execute store result score %current_damage uhcp_aug_durability run data get entity @s Inventory[{Slot:$(Slot)b}].components."minecraft:damage"
...
$execute store result score %max_damage uhcp_aug_durability run data get entity @s Inventory[{Slot:$(Slot)b}].components."minecraft:max_damage"
scoreboard players operation %damage uhcp_aug_durability /= %max_damage uhcp_aug_durability
$item modify entity @s container.$(Slot) uhcp:update_dur