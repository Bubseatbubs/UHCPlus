...
execute if score %blue ssiege_perk_sonar matches 2.. at @e[type=marker,tag=BLUE_BASE,limit=1] run effect give @a[team=red,distance=..40] glowing 10 0 true

# Red team sonar: Apply glowing to blue team players near red base (if red has sonar perk)
execute if score %red ssiege_perk_sonar matches 2.. at @e[type=marker,tag=RED_BASE,limit=1] run effect give @a[team=blue,distance=..40] glowing 10 0 true
