# 20 Gapples
execute at @s as @e[predicate=uhcp:targetable,distance=..6] unless score @s uhcp_team = @n[tag=UHCP_GoldBlock] uhcp_team run damage @s 24 minecraft:falling_block
execute at @s as @e[predicate=uhcp:targetable,distance=..6] unless score @s uhcp_team = @n[tag=UHCP_GoldBlock] uhcp_team run effect give @s minecraft:slowness 2 100 false
