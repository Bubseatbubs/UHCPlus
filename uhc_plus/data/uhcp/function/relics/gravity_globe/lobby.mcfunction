...
execute as @e[distance=..8,type=!#uhcp:inanimate_mobs] unless score @s uhcp_initStatus = @p[tag=UHCP_Owner] uhcp_initStatus at @s run particle minecraft:end_rod ~ ~ ~ 0.1 0.1 0.1 0.1 50 normal

execute as @a[distance=..8,team=!grace_period,scores={uhcp_initStatus=0..}] unless score @s uhcp_initStatus = @p[tag=UHCP_Owner] uhcp_initStatus run tag @s add UHCP_InflictedWithGravityGlobe
execute as @a[distance=..8,tag=UHCP_InflictedWithGravityGlobe] run function uhcp:relics/gravity_globe/gravity/normal
execute as @e[distance=..8,type=!minecraft:player,predicate=uhcp:targetable] unless score @s uhcp_initStatus = @p[tag=UHCP_Owner] uhcp_initStatus run data modify entity @s Motion[1] set value 1.75d
