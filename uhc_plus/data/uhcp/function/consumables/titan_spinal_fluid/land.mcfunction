...
execute as @e[type=!#uhcp:inanimate_mobs,distance=..7] at @s run tp ~ ~0.25 ~
execute as @e[type=!minecraft:player,predicate=uhcp:targetable,distance=..7] unless score @s uhcp_team = @p[tag=UHCP_Owner] uhcp_team run data modify entity @s Motion[1] set value 1.25d
execute as @e[type=!#uhcp:inanimate_mobs,distance=..7] at @s unless score @s uhcp_team = @p[tag=UHCP_Owner] uhcp_team run damage @s 9 minecraft:player_attack by @p[tag=UHCP_Owner]
