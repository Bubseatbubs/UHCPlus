...
# change this to utilize loot summoning function
execute as @e[type=minecraft:wolf,scores={uhcp_aug_count=1}] at @s run loot spawn ~ ~ ~ loot uhcp:augments/mans_best_friend
execute as @e[type=minecraft:wolf,scores={uhcp_aug_count=1}] at @s run particle minecraft:block{block_state:"minecraft:dirt"} ~ ~ ~ 0.5 0.5 0.5 1 50 normal
