...
execute as @e[type=player,distance=..6,scores={uhcp_initStatus=1..}] unless score @s uhcp_initStatus = @a[tag=UHCP_Owner,sort=nearest,limit=1] uhcp_initStatus at @s run tag @s add UHCP_InflictedWithGravityGlobe
execute as @e[type=player,distance=..6,scores={uhcp_initStatus=1..}] unless score @s uhcp_initStatus = @a[tag=UHCP_Owner,sort=nearest,limit=1] uhcp_initStatus at @s run attribute @s generic.gravity modifier add 95c5d624-7157-4cee-95eb-0d079cbfec99 "" -0.4 add_value
execute as @e[type=!player,type=!#uhcp:inanimate_mobs,distance=..10] unless score @s uhcp_initStatus = @a[tag=UHCP_Owner,sort=nearest,limit=1] uhcp_initStatus run data merge entity @s {Motion:[0.0,1.75,0.0]}
