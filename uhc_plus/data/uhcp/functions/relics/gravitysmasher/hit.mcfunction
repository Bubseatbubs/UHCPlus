advancement revoke @s only uhcp:augments/hurt_with_gravity_smasher
...
execute at @s as @e[type=player,distance=..6] unless score @s uhcp_team = @a[tag=UHCP_Owner,sort=nearest,limit=1] uhcp_team at @s run summon armor_stand ~ ~ ~ {Invulnerable:1b,Invisible:1b,Small:1b,Silent:1b,Motion:[0.0,1.0,0.0],Tags:["UHCP_Temp","UHCP_New"]}
execute store result score @e[distance=..6,type=armor_stand,tag=UHCP_New] uhcp_team run scoreboard players get @s uhcp_team
execute at @s as @e[type=player,distance=..6] unless score @s uhcp_team = @a[tag=UHCP_Owner,sort=nearest,limit=1] uhcp_team at @s run ride @s mount @e[tag=UHCP_Temp,type=armor_stand,sort=nearest,limit=1,distance=..1]
