execute store result bossbar minecraft:uhcp_gigantushealth value run data get entity @s Health
bossbar set minecraft:uhcp_gigantushealth players @a[distance=..32]
bossbar set minecraft:uhcp_gigantushealth name {"selector":"@s","color":"gold"}
...
execute if score %y_diff uhcp_t_heightDifference matches 2.. facing entity @p[tag=UHCP_GigantusTarget] feet run tp @s ^ ^ ^0.2 ~ ~20
execute if score %y_diff uhcp_t_heightDifference matches ..-2 facing entity @p[tag=UHCP_GigantusTarget] feet run tp @s ^ ^ ^0.2 ~ ~20
