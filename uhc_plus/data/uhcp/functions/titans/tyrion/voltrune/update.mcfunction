...
execute if score @s uhcp_game_time matches 118 as @a[gamemode=survival,distance=..12.5] at @s run execute at @a[distance=..16,gamemode=survival] run playsound minecraft:block.anvil.place master @s ~ ~4 ~ 100 1 1
execute if score @s uhcp_game_time matches 118 as @a[gamemode=survival,distance=..12.5] at @s run execute at @a[distance=..16,gamemode=survival] run summon minecraft:armor_stand ~ ~8 ~ {NoGravity:0b,Silent:1b,Small:1b,Invisible:1b,Tags:["UHCP_New","UHCP_BoneBlock","UHCP_Summon"],Passengers:[{id:"minecraft:block_display",NoGravity:0b,Tags:["UHCP_BBDisplay"],transformation:{left_rotation:[0f,0f,0f,1f],right_rotation:[0f,0f,0f,1f],translation:[-1.5f,-1.5f,-1.5f],scale:[3f,3f,3f]},Glowing:1b,shadow_radius:15f,shadow_strength:1f,block_state:{Name:"minecraft:gold_block"}}]}
execute as @a[gamemode=survival,distance=..12.5] run effect give @s minecraft:slowness 1 0 false
