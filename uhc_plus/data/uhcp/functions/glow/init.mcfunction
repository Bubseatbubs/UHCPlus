...
execute as @a[scores={uhcp_team=1..64}] if score @s uhcp_team = @a[tag=UHCP_GTrack,limit=1] uhcp_team at @s run tellraw @s [{"selector":"@a[tag=UHCP_GTrack,limit=1]","color":"gold"},{"text":" inflicted","color":"gray"},{"text":" Glowing","color":"white"},{"text":" on other teams!","color":"gray"}]
