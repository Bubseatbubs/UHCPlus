...
execute at @s unless entity @e[tag=!UHCP_Bee,tag=!UHCP_Owner,tag=!UHCP_TeamMember,type=!#uhcp:inanimate_mobs,distance=..8] run function uhcp:killsummon
execute at @s if score @s uhcp_team = @p[scores={uhcp_bee_stack=0..}] uhcp_team run tag @p[scores={uhcp_bee_stack=0..}] add UHCP_Owner
execute as @e if score @s uhcp_team = @a[tag=UHCP_Owner,sort=nearest,limit=1] uhcp_team run tag @s add UHCP_TeamMember
