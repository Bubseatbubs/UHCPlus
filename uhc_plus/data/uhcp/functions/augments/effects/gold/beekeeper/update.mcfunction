...
data merge entity @s {AngerTime:4000000,HasStung:0b}
...
execute at @p[tag=UHCP_Owner] run data modify entity @s AngryAt set from entity @e[tag=!UHCP_Bee,type=!#uhcp:inanimate_mobs,tag=!UHCP_Owner,tag=!UHCP_TeamMember,sort=nearest,limit=1] UUID
tag @a remove UHCP_Owner
tag @a remove UHCP_TeamMember
scoreboard players set @s uhcp_a_timer 10