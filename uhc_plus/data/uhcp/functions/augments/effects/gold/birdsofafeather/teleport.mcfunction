...
execute as @a[tag=!UHCP_BirdMan,gamemode=survival] at @s if score @a[tag=UHCP_BirdMan,limit=1,sort=nearest] uhcp_team = @s uhcp_team run tag @s add UHCP_BirdMaybeTarget
tag @r[tag=UHCP_BirdMaybeTarget] add UHCP_BirdTarget
tp @a[tag=UHCP_BirdMan,limit=1,sort=nearest] @r[tag=UHCP_BirdTarget]
