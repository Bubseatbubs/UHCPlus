...
execute as @e[type=!minecraft:player,tag=!UHCP_Invulnerable,team=!grace_period,predicate=uhcp:was_not_hurt_recently,type=!#uhcp:inanimate_mobs,distance=..2.5] unless score @s uhcp_team = @e[tag=UHCP_CurrentPanda,sort=nearest,limit=1] uhcp_team at @s run function uhcp:augments/effects/prismatic/kungfupanda/damage
