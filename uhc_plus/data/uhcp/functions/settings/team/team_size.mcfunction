...
execute if score %team_size uhcp_settings matches 1 run return run tellraw @s [{"text":"When randomizing teams, "},{"score":{"name":"%team_size","objective":"uhcp_settings"},"color":"gold"},{"text":" player will be on each team when\npossible."}]
