...
#Clear items
execute if score countdown tick matches 20 run kill @e[type=item]
...
execute if score speed status matches 1 run effect give @a[gamemode=survival] minecraft:speed 15 0 true
...
#Death
execute as @a[scores={death=1}] if score respawn status matches 0 run function game:death
